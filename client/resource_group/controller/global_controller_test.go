...
func TestGetResourceGroup(t *testing.T) {
	...
	t.Run("leader-change-uses-degraded-and-recovers", func(t *testing.T) {
		re := require.New(t)
		mockProvider := newMockResourceGroupProvider()
		controller := newController(t, mockProvider, WithDegradedRUSettings(degradedRUSettings))
		...
		realGroup := newResourceGroup("test-group", 1000000, 0)
		mockProvider.On("GetResourceGroup", mock.Anything, "test-group", mock.Anything).
			Return((*rmpb.ResourceGroup)(nil), wrapGetResourceGroupErr("test-group", errors.New(errs.NotLeaderErr))).
			Once()
		mockProvider.On("GetResourceGroup", mock.Anything, "test-group", mock.Anything).
			Return(realGroup, nil).
			Once()
	...
	t.Run("switch-group-to-nonexistent-target-does-not-degrade", func(t *testing.T) {
		re := require.New(t)
		mockProvider := newMockResourceGroupProvider()
		controller := newController(t, mockProvider, WithDegradedRUSettings(degradedRUSettings))

		missingTargetErr := wrapGetResourceGroupErr("rg_missing_target",
			status.Error(codes.Unknown, "[PD:resourcemanager:ErrGroupNotExists]the rg_missing_target resource group does not exist"))
		mockProvider.On("GetResourceGroup", mock.Anything, "rg_missing_target", mock.Anything).
			Return((*rmpb.ResourceGroup)(nil), missingTargetErr).
			Once()

		gc, err := controller.tryGetResourceGroupController(ctx, "rg_missing_target", false)
		re.Error(err)
		re.Same(missingTargetErr, err)
		re.Nil(gc)
		mockProvider.AssertNumberOfCalls(t, "GetResourceGroup", 1)
	})
