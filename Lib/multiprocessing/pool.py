...
class Pool(object):
    ...
    def __init__(self, processes=None, initializer=None, initargs=(),
        ...
        # on every active semaphore when the pool is terminating to let
        # task_handler wake up to stop. It's a set so that each iterator object
        # can efficiently deregister its semaphore when the iterator finishes.
...
class IMapIterator(object):
    ...
    def _detach_pool(self):
        if self._pool is not None:
            # `_pool` may already be None from a previous next()/_set() call
            self._pool._taskqueue_buffersize_semaphores.discard(
