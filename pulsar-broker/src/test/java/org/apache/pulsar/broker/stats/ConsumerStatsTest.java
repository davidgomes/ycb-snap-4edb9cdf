...
public class ConsumerStatsTest extends ProducerConsumerBase {
    ...
    public void testUnackedCountNonBatchAfterExpire(SubscriptionType subType) throws Exception {
        String topic = TopicName.get(newTopicName()).toString();
        String sub = "sub";
        int numMessages = 10;
     ...
     *
     * Flow: produce batch(batchSize=10) → consume all → ack 5 of 10 → expire → unacked should be 0.
     */
    @Test(dataProvider = "subscriptionTypes")
    ...
    private void expireAndVerifyUnackedDrained(String topic, String sub,
        ...
        // Trigger readMoreEntries to invoke removeAllUpTo. Flush so batched producers send immediately.
        producer.sendAsync("trigger".getBytes());
        producer.flush();
