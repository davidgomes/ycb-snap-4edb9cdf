...
func TestHandler(t *testing.T) {
	...
	t.Run("/api/v1/alerts", func(t *testing.T) {
		...
		lr = listAlertsResponse{}
		getResp(t, ts.URL+`/vmalert/api/v1/alerts?match[]={job="foo"}`, &lr, 200)
		if length := len(lr.Data.Alerts); length != 3 {
			t.Fatalf("expected 3 alerts got %d", length)
		}

		lr = listAlertsResponse{}
		getResp(t, ts.URL+`/api/v1/alerts?match[]={job="foo"}&datasource_type=prometheus`, &lr, 200)
		if length := len(lr.Data.Alerts); length != 2 {
			t.Fatalf("expected 2 alerts got %d", length)
		}

		getResp(t, ts.URL+`/api/v1/alerts?match[]={job=!"foo"}`, nil, 400)
		getResp(t, ts.URL+`/api/v1/alerts?match[]=sum(up)`, nil, 400)
	})
	...
	t.Run("/api/v1/rules&states", func(t *testing.T) {
		...
		check(`/api/v1/rules?match[]=sum(up)`, 400, 0, 0)
		check(`/vmalert/api/v1/rules?match[]={job="foo"}`, 200, 3, 3)
		check(`/api/v1/rules?match[]={job="foo"}`, 200, 3, 3)
		...
		check(`/api/v1/rules?match[]={job="foo"}&type=alert`, 200, 3, 3)
		check(`/api/v1/rules?match[]={job="foo"}&type=record`, 200, 0, 0)
